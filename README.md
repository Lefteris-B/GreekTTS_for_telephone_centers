...
This application bundles or relies on several third-party components, each under its own license. The summary below reflects what was published on the source repositories at the time of writing — verify the current license text on each linked page before deployment, especially for government or large-scale commercial use.
Models
...
Summary
For a normal commercial or government call-center deployment, the entire stack is usable, provided you comply with:

Meta's Llama 3.2 Acceptable Use Policy (no deception, no fraud, no impersonation without consent — see the policy before deploying for outbound calls).
